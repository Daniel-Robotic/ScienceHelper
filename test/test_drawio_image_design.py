...
from science_helper.image_processing import DrawioImageDesign
from science_helper.image_processing.enumerates import LabelMode, SignaturePosition


class MockDrawioImageDesign(DrawioImageDesign):  # noqa: D101
...
@pytest.fixture
def design():  # noqa: D103
    d = MockDrawioImageDesign()
    d._signature_label = LabelMode.CYRILLIC_LOWER
    return d


def test_add_numbering_creates_cell(design):  # noqa: D103
    design._add_numbering(image_w=320, image_h=270, label="1", parent_id="testparent")
    cell = next(
        (
            e
            for e in design._xml_root
            if e.attrib.get("parent") == "testparent"
            and e.attrib.get("id", "").endswith("-numbering")
        ),
        None,
    )
...
def test_preprocessing_image_adds_cells(design):  # noqa: D103
    ...
    numbering_cell = next(
        (
            e
            for e in design._xml_root
            if e.attrib.get("parent") == img_cell.attrib["id"]
            and e.attrib.get("id", "").endswith("-numbering")
        ),
        None,
    )
...
def test_generate_id_format():  # noqa: D103
    id_ = DrawioImageDesign._generate_id()
    assert id_.startswith("E__")
    assert id_.endswith("-1")


def test_add_axes_adds_group_and_elements(design):  # noqa: D103
    ...
    group_cell = next(
        (
            e
            for e in design._xml_root
            if e.attrib.get("style") == "group" and e.attrib.get("parent") == "1"
        ),
        None,
    )
    ...
    labels = [
        e
        for e in design._xml_root
        if e.attrib.get("parent") == group_cell.attrib["id"]
        and "text;" in e.attrib.get("style", "")
    ]
    ...
    arrows = [
        e
        for e in design._xml_root
        if e.attrib.get("parent") == group_cell.attrib["id"] and e.attrib.get("edge") == "1"
    ]
    assert len(arrows) == 2  # noqa: PLR2004


def test_layout_images_row_mode(design):  # noqa: D103
...
def test_layout_images_column_mode(design):  # noqa: D103
...
def test_united_images_combines_images(design):  # noqa: D103
...
def test_create_mx_cell_and_geometry(design):  # noqa: D103
    cell = design._create_mx_cell(
        id="test", value="testval", style="teststyle", vertex="1", parent="1"
    )
...
def test_add_label_creates_text_cell(design):  # noqa: D103
